...
export default class TemplateRenderer {
  ...
  constructor(options) {
    ...
    this.parsedTemplate = options.template ? parseTemplate(options.template) : null;
  ...
  bindRenderFns(context) {
    const renderer = this;['ResourceHints', 'State', 'Scripts', 'Styles'].forEach(type => {
  ...
  renderStyles(context) {
    const cssFiles = this.clientManifest ? this.clientManifest.all.filter(isCSS) : [];
    return (
      // render links for css files
      (cssFiles.length ? cssFiles.map(file => `<link rel="stylesheet" href="${this.publicPath}/${file}">`).join('') : '') + (
  ...
  renderState(context, options) {
    ...
    const autoRemove = process.env.NODE_ENV === 'production' ? ';(function(){var s;(s=document.currentScript||document.scripts[document.scripts.length-1]).parentNode.removeChild(s);}());' : '';
